            ...
            <div class="finance-wrapper">
                <div class="container-ingresos-neon">
                    <div class="ingresos-header">
                        ...
                        <span style="font-size: 0.65rem; background: var(--verde-verificado); color: #000; padding: 2px 6px; border-radius: 4px; font-weight: 900;">AUTOMÁTICO / MANUAL</span>
                    </div>
                    ...
                    <div class="sub-gastos-form">
                        <input type="text" id="ingresoConcepto" class="input-custom" placeholder="Ej: Carrera Extra, Propina..." style="margin-bottom:0; font-size:0.8rem; padding:8px;">
                        <input type="number" id="ingresoMonto" class="input-custom" placeholder="Valor ($)" style="margin-bottom:0; font-size:0.8rem; padding:8px; width:110px;">
                        <button class="btn-add-ingreso" onclick="agregarIngresoManualDev()" title="Agregar Ingreso Manual">
    ...
    <div class="modal-overlay" id="modalQR">
        <div class="modal-content">
            ...
            <div class="qr-image-container">
                <img src="https://scontent.fnva2-1.fna.fbcdn.net/v/t39.30808-6/770562384_986398197754607_2505745652231744957_n.jpg?stp=dst-jpg_tt6&cstp=mx2048x2048&ctp=s2048x2048&_nc_cat=100&ccb=1-7&_nc_sid=6ee11a&_nc_ohc=gd7NfdbVzPkQ7kNvwHGK7H5&_nc_oc=AdodzRLiDqI2QJ0eyibdjtPC62g-B7mwzSfhZwPT5YSVgRFiUc-SFX9VU8vyvPyh_0s&_nc_zt=23&_nc_ht=scontent.fnva2-1.fna&_nc_gid=k8J_OO78tWO4R0LQjIh65g&_nc_ss=7b2a8&oh=00_AQFSWRM0EvWIG-KzYiSuC3Z-MUjtZjPzdzkgr0a-xHl3og&oe=6A897443" alt="Código QR de Pago">
            </div>
    ...
    <script>
        ...
        function loginSocial(providerName) {
            ...
            window.firebaseFns.signInWithPopup(window.firebaseAuth, provider)
                .then((result) => {
                    manejarUsuarioAutenticado(result.user);
                }).catch((error) => {
        ...
        function enviarCodigoSMS() {
            ...
            window.firebaseFns.signInWithPhoneNumber(window.firebaseAuth, telefono, window.recaptchaVerifier)
        ...
        function agregarIngresoManualDev() {
            ...
            if (!concepto || isNaN(monto) || monto <= 0) {
                alert("Por favor ingresa un concepto y un valor válido de ingreso.");
                return;
            }
        ...
        function eliminarIngresoDev(id) {
            ...
            if (stats.historial) {
                ...
                if (index !== -1) {
                    if (stats.historial[index].resultado === 'CONFIRMADA' && stats.confirmadas > 0 && stats.historial[index].codigo !== "MANUAL") {
                        stats.confirmadas--;
                    }
        ...
        function renderizarListaIngresos(historial) {
            ...
            [...confirmados].reverse().forEach(item => {
                ...
                let etiquetaTipo = '🛵 [TRANSPORTE]';
                let detalleRuta = `${item.origen} ➔ ${item.destino}`;
                
                if (item.tipo === 'domicilio') {
                    etiquetaTipo = '📦 [DOMICILIO]';
                    detalleRuta = `${item.establecimiento} (${item.detalle}) ➔ ${item.origen}`;
                } else if (item.tipo === 'manual') {
                    etiquetaTipo = '💵 [MANUAL]';
                    detalleRuta = `${item.usuario}`;
                }
        ...
        function abrirModalQR() { document.getElementById("modalQR").classList.add("active"); }
        function cerrarModalQR() { document.getElementById("modalQR").classList.remove("active"); }
        function abrirModalTerminos() { document.getElementById("modalTerminos").classList.add("active"); }
        function cerrarModalTerminos() { document.getElementById("modalTerminos").classList.remove("active"); }
        ...
        function mostrarUIEsperaCliente() {
            ...
            document.getElementById("statusTitle").innerText = tipoServicioActual === 'domicilio' ? "Esperando confirmación del domiciliario..." : "Esperando confirmación del conductor...";
        ...
        function mostrarUIOfertaDesarrollador() {
            ...
            document.getElementById("statusTitle").innerText = tipoServicioActual === 'domicilio' ? "¡El domiciliario propone una nueva tarifa!" : "¡El conductor propone una nueva tarifa!";
        ...
        function mostrarUIAceptadoCliente() {
            ...
            document.getElementById("statusTitle").innerText = tipoServicioActual === 'domicilio' ? "¡Tu domicilio va en camino!" : "¡El conductor ya va por ti!";
        ...
        function mostrarUILlegadoCliente() {
            ...
            document.getElementById("statusTitle").innerText = tipoServicioActual === 'domicilio' ? "¡Domicilio entregado con éxito!" : "¡Conductor llegado a destino!";
        ...
        function cancelarServicio() {
            if (['SOLICITADO', 'OFERTA_DESARROLLADOR', 'ACEPTADO'].includes(estadoSolicitud)) {
        ...
        function actualizarVistaDev() {
            ...
            if (estadoSolicitud === 'SOLICITADO') {
                ...
                container.innerHTML = `
                    <div style="background: rgba(212,175,55,0.1); border: 2px solid var(--dorado-brillante); border-radius: 12px; padding: 14px; margin-bottom: 15px;">
                        ...
                        <div style="background:rgba(0,0,0,0.5); padding:10px; border-radius:8px; margin-bottom:10px; font-size:0.8rem;">
                            ...
                            ${htmlDetalles}
                            <p style="font-size: 0.9rem; color: var(--verde-verificado); margin-top: 6px;">
                                <strong>💰 Valor Propuesto:</strong> $${Number(costoRecorrido).toLocaleString('es-CO')}
                            </p>
                        </div>
                        
                        <div style="margin-bottom:10px;">
                            <label style="font-size:0.75rem; color:var(--dorado-brillante); font-weight:700; display:block; margin-bottom:4px;">Ajustar / Proponer Tarifa ($):</label>
                            <div style="display:flex; gap:6px;">
                                <input type="number" id="inputCostoDev" class="input-custom" value="${costoRecorrido}" style="margin-bottom:0; font-size:0.9rem; padding:8px;" min="4500" step="500">
                                <button class="btn-action btn-aceptar" onclick="ajestaryAceptarServicioDev()" style="width:130px; font-size:0.75rem; padding:8px;">
                                    Enviar / Aceptar
                                </button>
                            </div>
                        </div>
                        ...
                        <div style="display: flex; gap: 8px;">
                            <button class="btn-action btn-whatsapp" onclick="enviarWhatsAppDev()" style="font-size:0.8rem; padding:10px;">
                                <span class="material-icons" style="font-size:18px;">chat</span> WhatsApp
                            </button>
                            <button class="btn-action btn-cancelar" onclick="cancelarServicio()" style="font-size:0.8rem; padding:10px; margin-top:0;">
                                <span class="material-icons" style="font-size:18px;">cancel</span> Rechazar
                            </button>
                        </div>
                    </div>
                `;
                ...
                container.innerHTML = `
                    <div style="background: rgba(255,170,0,0.1); border: 2px solid #ffaa00; border-radius: 12px; padding: 14px; margin-bottom: 15px; text-align:center;">
                        <span class="material-icons" style="font-size:32px; color:#ffaa00;">hourglass_top</span>
                        <h4 style="color:#ffaa00; margin:6px 0;">Esperando respuesta del cliente...</h4>
                        <p style="font-size:0.8rem; color:var(--texto-secundario);">Se le propuso la nueva tarifa de <strong>$${Number(costoRecorrido).toLocaleString('es-CO')}</strong> al usuario.</p>
                        <button class="btn-action btn-cancelar" onclick="cancelarServicio()" style="margin-top:10px;">Cancelar Servicio</button>
                    </div>
                `;
            } else if (estadoSolicitud === 'ACEPTADO') {
                const usr = usuarioActual || { nombre: "Cliente", telefono: "No registrado" };
                container.innerHTML = `
                    <div style="background: rgba(0,255,136,0.1); border: 2px solid var(--verde-verificado); border-radius: 12px; padding: 14px; margin-bottom: 15px;">
                        <h4 style="color:var(--verde-verificado); margin-bottom:6px;">✅ SERVICIO EN CURSO / ACEPTADO</h4>
                        <div class="codigo-badge-box" style="margin-bottom:8px;">
                            <div style="font-size: 0.68rem; color: var(--texto-secundario);">CÓDIGO DE VERIFICACIÓN</div>
                            <div class="codigo-num" style="font-size: 1.4rem;">${codigoServicio}</div>
                        </div>
                        <p style="font-size:0.85rem; color:#fff; margin-bottom:4px;"><strong>Cliente:</strong> ${usr.nombre} (Tel: ${usr.telefono})</p>
                        <p style="font-size:0.85rem; color:var(--verde-verificado); margin-bottom:10px;"><strong>Valor Acordado:</strong> $${Number(costoRecorrido).toLocaleString('es-CO')}</p>
                        
                        <div style="display:flex; gap:8px;">
                            <button class="btn-action btn-whatsapp" onclick="enviarWhatsAppDev()" style="font-size:0.8rem; padding:10px;">
                                <span class="material-icons" style="font-size:18px;">chat</span> Contactar WhatsApp
                            </button>
                            <button class="btn-action btn-cancelar" onclick="cancelarServicio()" style="font-size:0.8rem; padding:10px; margin-top:0;">
                                Finalizar / Cancelar
                            </button>
                ...
                container.innerHTML = `
                    <div style="background: rgba(0,210,255,0.1); border: 2px solid #00d2ff; border-radius: 12px; padding: 14px; margin-bottom: 15px; text-align:center;">
                        <span class="material-icons" style="font-size:32px; color:#00d2ff;">task_alt</span>
                        <h4 style="color:#00d2ff; margin:6px 0;">¡Servicio Finalizado / Llegado!</h4>
                        <p style="font-size:0.8rem; color:var(--texto-secundario);">El usuario ha notificado la llegada a destino.</p>
                        <button class="btn-action btn-aceptar" onclick="cancelarServicio()" style="margin-top:10px;">Nuevo Servicio</button>
                    </div>
                `;
            } else {
                container.innerHTML = `
        ...
        function ajestaryAceptarServicioDev() {
            const inputDev = document.getElementById("inputCostoDev");
            const nuevoPrecio = inputDev ? parseFloat(inputDev.value) : costoRecorrido;
